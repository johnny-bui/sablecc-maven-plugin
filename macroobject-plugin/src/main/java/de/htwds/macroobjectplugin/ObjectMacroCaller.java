...
public class ObjectMacroCaller extends AbstractMojo {
	...
	private String[] parseArgument(Map m) {
		...
		if (p!=null){
			...
			if (isPackageNameValue(packagename)){
	...
	boolean isPackageNameValue(String pn){
		if (!isOptionValid(pn)){
			return false;
		}
